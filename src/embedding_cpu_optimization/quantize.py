...
def load_qasper_calibration_set(sample_size) -> Dataset:
    ...
    random_queries = [
        random.sample(train_set[x]["qas"]["question"], 1)[0] for x in random_samples
    ]
...
def quantize(model_name, output_path, calibration_set):
    ...
    quantization_config = PostTrainingQuantConfig(
        approach="static", backend="ipex", domain="nlp"
    )
    quantizer.quantize(
...
def validate_model(model_path, task, pooling, is_optimized=False):
    ...
    if is_optimized:
        ...
        model = EmbedderModelMTEB(
            qm, tokenizer, pooling=pooling, query_prompt=query_prompt
        )
    else:
...
if __name__ == "__main__":
    ...
    # quantize('intfloat/multilingual-e5-base')
